...
const ProjectCard = ({ index, title, desc, stack, number, artClass, Icon, delay, overlayClass, iconColor }) => {
    ...
    return (
        <div ref={cardRef} className={`project-card scroll-anim opacity-0 translate-y-20 perspective-[1000px] ${index % 2 !== 0 ? 'md:mt-32' : ''}`} data-delay={delay}>
            <div ref={innerRef} className="project-card-inner flex flex-col h-full transform-style-3d will-change-transform translate-z-[30px] border border-white/10 bg-[#0a0a0a] rounded-3xl overflow-hidden hover:border-[#C4B5FD] transition-colors duration-500 shadow-[0_18px_40px_rgba(124,58,237,0.08)]">
                <div className="project-image-wrapper aspect-[16/10] bg-[#0a0a0a]/5 flex items-center justify-center relative overflow-hidden group">
                ...
                <div className="p-6 mt-auto">
                    <div className="flex justify-between items-end">
                        <div>
                            <h3 className="text-3xl font-display font-bold mb-2 text-white">{title}</h3>
                            <p className="text-gray-400">{desc}</p>
                        </div>
                        <a href="#" className={`w-14 h-14 shrink-0 rounded-full border border-white/10 flex items-center justify-center transition-all interactive-hover text-white
                            ${index===0 ? 'hover:bg-purple-600 hover:border-[#7C3AED] hover:text-white' : 
                              index===1 ? 'hover:bg-purple-800 hover:border-[#C4B5FD]' :
                              index===2 ? 'hover:bg-[#DDD6FE] hover:border-white/10' : 'hover:bg-purple-600 hover:border-[#7C3AED] hover:text-white'}`}>
...
const Exhibicion = () => {
    ...
    const iconColors = ["text-purple-400", "text-[#A855F7]", "text-[#6D28D9]", "text-purple-400"];
    const overlayClasses = [
        ...
        <><div key="1a" className="art-layer absolute w-full h-[1px] bg-purple-600/25 rotate-45"></div><div key="1b" className="art-layer absolute w-full h-[1px] bg-purple-600/25 -rotate-45"></div></>,
        <div key="2" className="art-layer absolute inset-8 border border-dashed border-[#7C3AED]/25 rounded-full"></div>,
        <div key="3" className="art-layer absolute w-[80%] h-[80%] bg-purple-800/30 blur-2xl rounded-full"></div>,
    ];
    ...
    return (
        <section id="exhibicion" className="py-20 px-6 md:px-12 lg:px-24 relative z-10 w-full">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-20 w-full">
                ...
                <p className="text-gray-400 max-w-sm md:mb-4 text-lg">{contentMeta.archiveDesc}</p>
            </div>
